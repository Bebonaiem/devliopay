...
@php $symbol = $invoice->currency->symbol ?? $defaultCurrencySymbol; @endphp
                        ...
                        <tbody class="divide-y divide-white/5">
                            @if(isset($invoice->items) && count($invoice->items) > 0)
                                ...
                                <tr class="hover:bg-white/[0.02]">
                                    ...
                                    <td class="px-6 py-4 text-sm font-semibold text-right">{{ $symbol }}{{ number_format(($item->amount ?? 0) * ($item->quantity ?? 1), 2) }}</td>
                                </tr>
                                @endforeach
                            @else
            ...
            <div class="glass rounded-2xl p-6">
                ...
                <div x-show="!stripeReady" class="space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <label class="flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all"
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">Available: {{ $symbol }}{{ number_format(auth()->user()->balance ?? 0, 2) }}</p>
                                </div>
                            </label>
                            @endif
                    ...
                    <button @click="startPayment()" :disabled="processing" class="btn-primary w-full py-3 rounded-xl text-sm font-bold text-white shadow-lg shadow-brand-500/20 inline-flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                        <span x-show="!processing" class="inline-flex items-center gap-2"><i data-lucide="credit-card" class="w-4 h-4"></i> Pay {{ $symbol }}{{ number_format($invoice->total ?? 0, 2) }}</span>
                ...
                <div x-show="stripeReady" class="space-y-5">
                    ...
                    >
                        <span x-show="!processing" class="inline-flex items-center gap-2"><i data-lucide="lock" class="w-4 h-4"></i> Pay {{ $symbol }}{{ number_format($invoice->total ?? 0, 2) }}</span>
                        <span x-show="processing" class="inline-flex items-center gap-2"><svg class="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg> Processing...</span>
                    </button>
            ...
            <div class="glass rounded-2xl p-6" x-show="stripeReady">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm font-medium">{{ $symbol }}{{ number_format($invoice->tax, 2) }}</span>
                    </div>
                    @endif
                    @if(($invoice->credit ?? 0) > 0)
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-400">Credit</span>
                        <span class="text-sm font-medium text-emerald-400">-{{ $symbol }}{{ number_format($invoice->credit, 2) }}</span>
                    </div>
                    @endif
                    <hr class="border-white/5">
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-semibold">Total</span>
                        <span class="text-lg font-black">{{ $symbol }}{{ number_format($invoice->total ?? 0, 2) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="glass rounded-2xl p-6" x-show="!stripeReady">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm font-medium">{{ $symbol }}{{ number_format($invoice->subtotal ?? $invoice->total ?? 0, 2) }}</span>
                    </div>
                    @if(($invoice->tax ?? 0) > 0)
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-400">Tax</span>
