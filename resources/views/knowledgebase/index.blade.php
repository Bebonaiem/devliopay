...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="max-w-xl mx-auto mb-10">
        <div class="relative">
            ...
            <input type="text" placeholder="Search articles..." class="w-full bg-white/[0.03] border border-white/10 rounded-2xl pl-12 pr-4 py-4 text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:border-brand-500/50 focus:ring-1 focus:ring-brand-500/20">
        </div>
    </div>
    ...
    @if(isset($categories) && count($categories) > 0)
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <a href="{{ route('knowledgebase.show', $category->slug) }}" class="glass rounded-2xl p-6 hover:border-brand-500/20 transition-all group card-hover">
                <div class="flex items-center gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="text-base font-bold group-hover:text-brand-400 transition-colors">{{ $category->name }}</h3>
                        <p class="text-xs text-gray-500 mt-0.5">{{ $category->articles_count ?? $category->articles->count() ?? 0 }} articles</p>
                    </div>
