            ...
            @foreach($healthChecks as $key => $check)
                <div class="bg-white dark:bg-white/5 rounded-xl p-6 shadow-sm border-l-4 {{ match($check['status']) { 'success' => 'border-green-500', 'warning' => 'border-yellow-500', 'danger' => 'border-red-500', default => 'border-gray-500' } }}">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <div class="text-xs text-gray-500">{{ $check['label'] }}</div>
                            <div class="text-sm font-semibold {{ match($check['status']) { 'success' => 'text-green-600', 'warning' => 'text-yellow-600', 'danger' => 'text-red-600', default => 'text-gray-600' } }}">{{ $healthLabel($check['status']) }}</div>
                        </div>
                    </div>
                </div>
            @endforeach
                    ...
                    @foreach($healthChecks as $key => $check)
                        <div class="bg-white dark:bg-white/5 rounded-xl p-6 shadow-sm">
                            <div class="flex items-center gap-2 mb-4">
                                ...
                                <span class="ml-auto text-xs {{ match($check['status']) { 'success' => 'text-green-600', 'warning' => 'text-yellow-600', 'danger' => 'text-red-600', default => 'text-gray-500' } }}">{{ $healthLabel($check['status']) }}</span>
                            </div>
                            <div class="space-y-2">
                                @foreach($check['checks'] as $sub)
                                    <div class="flex items-center justify-between text-sm">
                                        ...
                                        <span class="flex items-center gap-1.5">
                                            ...
                                            @endif
                                            <span class="{{ $sub['ok'] ? 'text-gray-700 dark:text-gray-300' : 'text-red-600' }}">{{ $sub['info'] }}</span>
                                        </span>
                                    </div>
                                @endforeach
                            </div>
                ...
                <div class="bg-white dark:bg-white/5 rounded-xl p-6 shadow-sm">
                    <div x-data="{ tab: 'orders' }">
                        <div class="flex items-center gap-1 mb-4 border-b border-gray-200 dark:border-white/10 pb-3">
                            ...
                            <button @click="tab = 'tickets'" :class="{ 'bg-gray-100 dark:bg-white/10 font-medium': tab === 'tickets' }" class="px-3 py-1.5 text-sm rounded-lg transition text-gray-600 dark:text-gray-400">Recent Tickets</button>
                            <button @click="tab = 'transactions'" :class="{ 'bg-gray-100 dark:bg-white/10 font-medium': tab === 'transactions' }" class="px-3 py-1.5 text-sm rounded-lg transition text-gray-600 dark:text-gray-400">Recent Payments</button>
                        </div>
                        ...
                        <div x-show="tab === 'orders'" class="space-y-2">
                            @forelse($recentActivity['recent_orders'] as $order)
                                <a href="{{ route('filament.admin.resources.orders.edit', $order['id']) }}" class="flex items-center justify-between p-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-white/5 transition text-sm group">
                                    <div>
                                        <span class="font-medium text-gray-900 dark:text-gray-100">{{ $order['number'] }}</span>
                                        <span class="text-gray-500"> - {{ $order['user'] }}</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="font-medium text-gray-700 dark:text-gray-300">{{ $currencySymbol }}{{ number_format($order['total'], 2) }}</span>
                        ...
                        <div x-show="tab === 'tickets'" class="space-y-2">
                            @forelse($recentActivity['recent_tickets'] as $ticket)
                                <a href="{{ route('filament.admin.resources.ticket-threads.edit', $ticket['id']) }}" class="flex items-center justify-between p-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-white/5 transition text-sm group">
                                    <div>
                                        ...
                                        <span class="text-gray-500"> - {{ $ticket['subject'] }}</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                            ...
                            @empty
                                <p class="text-gray-500 text-sm text-center py-4">No recent tickets</p>
                            @endforelse
                        </div>
                        ...
                        <div x-show="tab === 'transactions'" class="space-y-2">
                            @forelse($recentActivity['recent_transactions'] as $txn)
                                <div class="flex items-center justify-between p-2.5 rounded-lg text-sm">
                                    <div>
                                        <span class="text-gray-700 dark:text-gray-300">{{ $txn['user'] }}</span>
                                        <span class="text-gray-400 text-xs ml-1 uppercase">({{ $txn['gateway'] }})</span>
                                    </div>
                                    <div class="flex items-center gap-3">
                                        <span class="font-medium text-green-600">{{ $currencySymbol }}{{ number_format($txn['amount'], 2) }}</span>
                                        <span class="text-gray-400 text-xs">{{ $txn['date'] }}</span>
                                    </div>
                                </div>
                            @empty
                                <p class="text-gray-500 text-sm text-center py-4">No recent payments</p>
                            @endforelse
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                    ...
                    <div class="space-y-3">
                        @php
                            $metrics = [
                                ...
                                ['label' => 'Total Users', 'value' => number_format($stats['total_users']), 'color' => 'text-gray-900 dark:text-gray-100'],
                                ...
                                ['label' => 'New This Month', 'value' => number_format($stats['new_users_this_month']) . ' users / ' . number_format($stats['new_orders_this_month']) . ' orders', 'color' => 'text-gray-900 dark:text-gray-100'],
                            ];
                        @endphp
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            <div class="flex justify-between text-sm mb-1">
                                ...
                                <span class="font-semibold text-gray-900 dark:text-gray-100">{{ $currencySymbol }}{{ number_format($stats['revenue_this_month'], 2) }}</span>
                            </div>
                        ...
                        <div>
                            <div class="flex justify-between text-sm mb-1">
                                ...
                                <span class="font-semibold text-gray-900 dark:text-gray-100">{{ $currencySymbol }}{{ number_format($stats['revenue_last_month'], 2) }}</span>
                            </div>
                            <div class="w-full h-2.5 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
                ...
                <div class="bg-white dark:bg-white/5 rounded-xl p-6 shadow-sm">
                    ...
                    <div class="grid grid-cols-3 gap-2 text-center text-sm">
                        <div>
                            <div class="text-gray-500 text-xs">Used</div>
                            <div class="font-semibold text-gray-900 dark:text-gray-100">{{ $systemInfo['disk_usage']['used'] }}</div>
                        </div>
                        <div>
                            <div class="text-gray-500 text-xs">Free</div>
                            <div class="font-semibold text-gray-900 dark:text-gray-100">{{ $systemInfo['disk_usage']['free'] }}</div>
                        </div>
                        <div>
                            <div class="text-gray-500 text-xs">Total</div>
                            <div class="font-semibold text-gray-900 dark:text-gray-100">{{ $systemInfo['disk_usage']['total'] }}</div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        @foreach($systemInfo['extensions'] as $name => $loaded)
                            <div class="flex items-center text-sm">
                                ...
                                <span class="{{ $loaded ? 'text-gray-700 dark:text-gray-300' : 'text-gray-400' }}">{{ $name }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        @foreach($sys as $s)
                            <div class="flex justify-between py-1 {{ !$loop->last ? 'border-b border-gray-100 dark:border-white/5' : '' }}">
                                ...
                                <span class="font-medium text-gray-900 dark:text-gray-100 text-right">{{ $s[1] }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
